...
class Utilities {
    ...
    // Allow scrolling if nav clicked or tab button pressed
    allowScroll = () => {
    ...
    // Remove diacritical marks; found here:
...
class Nav {
    ...
    closeBtnToggle = () => {
        // Update nav button depending on open state
    ...
    // Add classes to elements affected by nav position
...
class Headings {
    ...
    // Create minified version of all headings
    create = () => {
        for (const heading of this.all) {
            const { innerText: text, tagName, offsetTop } = heading;

            // Remove puntuation, replace diacritics with non-diacritics
    ...
    // Create mapping of new heading offsets
    refreshOffsets = () => {
...
class HashLinks {
    ...
    // Create hash links to headings
    create = () => {
        for (let i = 0; i < headings.list.length; i++) {
            const heading = headings.list[i];
            const { text, href } = heading;
            const message = 'CLiquez pour copier le lien';

            // Wrap the header in a span tag
            headings.all[i].innerHTML = `<span>${text}</span>`;

            // Create hash link before heading
...
class TableOfContents {
    ...
    // Highlight heading in toc list on scroll
    highlight = () => {
        const { list } = headings;
        ...
        for (let i = 0; i < num; i++) {
            ...
            const n = i + 1 < num ? i + 1 : i;
            const nextOffset = list[n].offsetTop;

            // Highlight and scroll to toc item if within scroll window
            if (scrollPosition > offsetTop && scrollPosition < nextOffset) {
                ...
                const nearBot = thisItem.offsetTop > this.element.scrollHeight - scrollPortion;

                listScrollLocation = nearTop ? 0
                    : nearBot ? this.element.scrollHeight
            ...
            const nearBot = document.documentElement.scrollHeight - (window.innerHeight + window.pageYOffset) < 50;
...
class FontSize {
    ...
    change = (thisBtn, thatBtn, increment) => {
        ...
        if (
            Math.abs(state.fontInt) > this._max ||
            Math.abs(increment) > this._max
        ) {
            return;
        }
        ...
        thisBtn.disabled = (Math.abs(state.fontInt) >= this._max) && true;
    ...
    set = userSetting => {
        ...
        if (Math.abs(increment) > this._max) {
            return;
        }
...
window.addEventListener('load', () => {
    ...
    setTimeout(() => state.navOpen ? nav.open() : nav.close(), 100);
...
window.addEventListener('resize', nav.closeBtnToggle);
window.addEventListener('resize', readingBar.setProgress);

window.addEventListener('scroll', scrollToTop.toggleBtn);
window.addEventListener('scroll', toc.highlight);
window.addEventListener('scroll', readingBar.setProgress);
...
theme.btn.addEventListener('click', () => theme.change(theme.classes.dark));
fontSize.decreaseBtn.addEventListener('click', () => fontSize.decrease());
fontSize.increaseBtn.addEventListener('click', () => fontSize.increase());
